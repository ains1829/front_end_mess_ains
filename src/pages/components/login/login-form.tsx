...
export function LoginForm({
  ...
  const login_query = useLogin();
  const navigate = useNavigate();
  const [error, seterror] = useState<any | undefined>();
  const { register, handleSubmit, reset } = useForm<AuthType>();
  const SubmitLogin: SubmitHandler<AuthType> = async (data) => {
    const reponse = await login_query.mutateAsync(data);
    console.log(reponse);
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-6">
        ...
        <Button variant="outline" className="w-full items-center flex">
          <svg
